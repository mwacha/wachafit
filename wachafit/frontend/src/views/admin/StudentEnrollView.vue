<template>
  <AppLayout>
    <div class="p-6 max-w-lg">
      <h1 class="text-2xl font-bold mb-6">Matricular Aluno</h1>
...
async function submit() {
  ...
  if (!form.value.studentEmail || !form.value.planId) {
  ...
  try {
    ...
    await membershipService.createSubscription(student.id, {
      planId: form.value.planId,
      startedAt: form.value.startedAt,
    })
    successMsg.value = `Aluno ${student.name} matriculado com sucesso!`
    form.value = { studentEmail: '', planId: '', startedAt: new Date().toISOString().slice(0, 10) }
  } catch (e: any) {
    errorMsg.value = e.response?.data?.error ?? 'Erro ao matricular aluno.'
