    ...
    <div class="view-wrap">
      ...
      <div class="wizard-card">
        ...
        <div v-if="currentStep === 0" class="step-content">
          <h2 class="step-title"><i class="pi pi-user" /> Conta do Aluno</h2>
          <div class="field-grid">
            <div class="field col-full">
              <label>E-mail *</label>
              <InputText v-model="account.email" type="email" placeholder="aluno@email.com" class="w-full"
                @blur="lookupUser" />
              <small v-if="userFound" class="found-msg"><i class="pi pi-check-circle" /> Aluno encontrado: {{ userFound.name }}</small>
              <small v-if="userNotFound" class="hint-msg">Aluno não cadastrado — preencha os dados abaixo para criar</small>
            </div>
            <template v-if="userNotFound">
              ...
              <div class="field">
                ...
                <InputText v-model="account.password" type="password" placeholder="Senha" class="w-full" />
              </div>
            </template>
            <div class="field">
              <label>Plano *</label>
              <Select v-model="account.planId" :options="plans" optionLabel="name" optionValue="id"
                placeholder="Selecione um plano" class="w-full" />
            </div>
            <div class="field">
              <label>Data de Início *</label>
              <InputText v-model="account.startedAt" type="date" class="w-full" />
            </div>
          </div>
        </div>
        ...
        <div class="step-nav">
          ...
          <Button v-if="currentStep < steps.length - 1"
            ...
            @click="nextStep" />
          <Button v-else
            label="Concluir Cadastro" icon="pi pi-check" iconPos="right"
...
const userFound = ref<AdminUser | null>(null)
const userNotFound = ref(false)
let resolvedStudentId = ''

// ---- Form state ----
const account = ref({ email: '', name: '', password: '', planId: '', startedAt: new Date().toISOString().slice(0, 10) })
...
async function lookupUser() {
  userFound.value = null; userNotFound.value = false
  if (!account.value.email) return
  try {
    const users = await userService.list({ role: 'STUDENT' })
    const found = users.find(u => u.email === account.value.email)
    if (found) { userFound.value = found; resolvedStudentId = found.id }
...
function nextStep() {
  ...
  if (currentStep.value === 0) {
    if (!account.value.email) { errorMsg.value = 'Informe o e-mail do aluno.'; return }
    if (!account.value.planId) { errorMsg.value = 'Selecione um plano.'; return }
...
async function submit() {
  ...
  try {
    ...
    if (!resolvedStudentId) {
      errorMsg.value = 'Aluno não identificado. Verifique o e-mail.'; return
    }

    // 2. Matrícula
    await membershipService.createSubscription(resolvedStudentId, {
      planId: account.value.planId,
      startedAt: account.value.startedAt,
    })

    // 3. Perfil (tenta criar; se já existe, atualiza)
    const profileData = {
    ...
    const existing = await profileService.getStudentProfile(resolvedStudentId)
    if (existing) await profileService.updateStudentProfile(resolvedStudentId, profileData)
    else await profileService.createStudentProfile(resolvedStudentId, profileData)
...
/* Found / hint messages */
.found-msg { color: #059669; font-size: 12px; display: flex; align-items: center; gap: 4px; margin-top: 4px; }
.hint-msg { color: var(--neutral-500); font-size: 12px; margin-top: 4px; }
